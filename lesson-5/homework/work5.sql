...
from Products;
--bu yerda Products tabledagi ProductName ustunni Name qilib ozgartirdim

--2.
select *
from Customers AS Client;
--bu yerda Customers jadvaliga vaqtinchalik Client  deb nom beradi, bu alias uzun nomli jadvallarda ishlashda qulaylik yaratadi 
--agar Client.FirstName deb yozilda bu Customers.FirstName degani boladi

--3.
select ProductName from Products
Union 
select ProductName from Products_Discounted
--bu ikkida jadvaldagi ProductName ustunini birlashtiradi va natijadagi takroriy qiymatlarni olib tashlaydi

--4.
select ProductName from Products
INTERSECT 
select ProductName from Products_Discounted
--bu yerda ikkala jadvaldagi ustunlardagi qaytarilgan qiymatni natijaga chiqaradi

--5. 
select distinct ISNULL(FirstName, '') + ' ' + ISNULL(LastName, '') as FullName, Country 
from Customers;
--bu yerda Customers jadvalida FirstName va LastName ustunlari bor bu 2 tasini umumiy natija qili FullName qilib oldim va ISNULL dan foydalandim chunki birortasi NULL bolsa FullName da Null bolib chiqib qoladi 
--disticnt oldin aytganimday har bir FullName ni natijalarni takrorlamasdan mamlakatlarini kombinatsiyalarini chiqarib beradi
...
from Products;
-- ProductName va Price ustunlarini tanlayabmiz agar narxi 1000 dan katta bolsa HIGH chiqar agar katta bolmasa LOW chiqar deb buyruq beryabmiza
--PriceCategory bu yerda HIGH yokida LOW chiqadigan ustun nomi

--7.
SELECT ProductName, StockQuantity,
IIF(StockQuantity > 100, 'Yes', 'No') AS InStock --alias berish bu chiqadigan ustunga vaqtinchalik nom berish
FROM Products_Discounted;
--IIF bu yerda if-else buyrugi hisoblanadi shartga qarab natija chiqaradi
--InStok bu yerda yes no chiqadigan ustun  nomi boladi

--CASE - kop shartlarni tekshiradi, juda moslashuvchan, sintaksisi lekin biroz uzunroq har qanday sql server versiyasida mavjud
--IIF faqat bitta shatni IF-ELSE ni tekshiradi, sodaroq, sintaksisi juda qisqa va oddiy, sql server 2022 dan keyingi versiyalarida bor bu

--8. bu savol 3 savol bilan bir xil
select ProductName from Products
Union 
select ProductName from Products_Discounted

--3 savol bilan 4 savol bir biriga yaqin bolgan savollar bittasida UNION ikkinchisida INTERSECT ikkalasini farqi bittasi natijalarni chiqaradi takrorlangan name ni bir marta yozadi bu UNION, ikkinchisi faqat takrorlangan name ni chiqaradi bu INTERSECT
...
--bu yerda SELECT * dan yani barchaustunlardan foydalanadi birinchi jadvaldagi biror ustunni ikkinchi jadvaldagi aynan oshanga oxshash ustundan yoq bolgan mahsuloatlarni chiqaradi

SELECT ProductName, Category FROM Products
EXCEPT
SELECT ProductName, Category FROM Products_Discounted;
--bu yerda faqat  ProductName, Category ustunlaridan foydalaniladi birirnchi jadvaldagi ProductName ustini ikkinchi jadvaldagi ProductName ustinidagi yoq narsalarni chiqaradi

--10.
select Price,
IIF(Price >1000, 'Expensive', 'Affordable') as Eqiual_Price
from Products
--bu yerda IIF dan foydalangan holda price narxi 1000 dan katta bolsa Expensive kichik bolsa Affordable chiqar va natijani ustini Equal_Price qilib qoy deb buyruq berildi

--11.
select *
from Employees
where Age < 25 and Salary > 60000;
-- jadvaldagi Age 25 dan kichik bolganlarni ham Salary 60000 dan katta bolganlarni  ham chiqaradi 
--agar OR orniga AND qoyganimizda Age 25 dan kichik bolganlarni  chiqaradi

--12.
update Employees
set Salary = Salary * 1.10
where DepartmentName = 'HR' or EmployeeID = 5;
--Employees jadvalni yangilayabmiz agar DepartmentName HR bolsa yoki EmployeeID 5 bolsa salaryni 10 % oshiradi
...
from Sales;
-- agar 500 dan katta bolsa Top tier va 200 500 oraliqda bolsa Mid tier aks holda Low tier chiqar deb beryabmiz bu yerda 3 ta ustun qoshyabman aniqroq korini uchun jadvalni 

--14.
SELECT CustomerID FROM Orders
EXCEPT
SELECT CustomerID FROM Sales;
--EXCEPT orqali biz faqat Ordersda bor-u, lekin Salesda yo‘q mijozlarni topamiz
