...
FROM Products;
--Products jadvalidagi Category ustinidagi qatnashgan mahsulotlarni chiqarish uchun SELECT DISTINCT
...
--bu yerda Products jadvaldagi narxi PRICE 100 katta bolgan bolgan mahsulatlarni korsatadi
...
--bu yerda order by tartiblab berish uchun
--ASC bu yreda osish  tartibida eng arzonida qimmatiga qarab tartiblaydi
--DESC ham bor faqat u kamayish tartibida qimmatdan azronga qarab tartiblaydi
...
--  bunda salary 60000 dan katta bolgan HR larni chiqaradi

SELECT *
FROM Employees
where Salary >=60000 OR DepartmentName = 'HR'

-- bu yerda ikkala buyruqni ham bajaradi

--7.
select Email, ISNULL(Email, 'noemail@example.com') AS Email
FROM Employees;
...
--bu yerda price 50 10 oraligida chiqaradi 
...
--ProductName va Category ustunlari alifbo tartibiga teskariz tarzda tartiblanadi 

--11.
select top 10 *
from Products
order by Price desc;
--bu yerda top 10 yani Price ni kamayish tartibida tartiblagandagi 10 ta qiymatni korsatadi

--12.
SELECT COALESCE(FirstName, LastName) AS FullName
FROM Employees;
--bu yerda birinchi First name chiqaradi agar first name NULL bolsa Last name ni chiqaradi

--13. 
select distinct Category, Price 
from Products;
--select distinct buyrugi bu bir marta yozilgan qiymatlarni takrorlamsadan korsatish

--14.
SELECT *
FROM Employees
WHERE (Age BETWEEN 30 AND 40) OR DepartmentName = 'Marketing';
--age 30 40 bolgangan qiymatlarni chiqar yoki departmentname Marketing bolgan qiymatlarni chiqar yani ikkasi ham bolsa bajardi
...
--Salary boyicha kamayish boyicha tartiblab 10 qatordan keyin 10 qatorni korsatish buyrugi berildi
...
--Products tableda Price 1000dan katta yoki teng va StokQuantity 50 da katta bolganlarni osish ASC boyicha tartiblash 

--17.
SELECT *
FROM Products
WHERE ProductName LIKE '%e%';
--%e% productName ustundagi mahsulotlarni orasida e harfi bolsa chiqar degan cammanda berilgan, agar buni E% deb berganimizda bosh harfi E bolganlarni chiqarardi, agar %e deb bersak oxirgi harfi e bn tugaganlari chiqaradi

--18.
SELECT *
FROM Employees
WHERE DepartmentName IN ('HR', 'IT', 'Finance');
--IN bir nechta qiymatlarni filtirlash uchun ishlatiladi bu yerda HR IT FINANCE qiymatlarini filtirladik
...
FROM Products
WHERE Price > 50;
--Price 50 dan yuqori bolgan 3 ta ustinni distinct qilib yani bir marta takrorlamasdan chiqarib berdi

--23. 
SELECT *
FROM Products
WHERE Price < (SELECT AVG(Price) * 0.1 FROM Products);
--bu yerda osha ortacha qiymatni 10% dan kam bolganlarni chiqaradi

SELECT AVG(Price)  FROM Products
--bu Price ni Avarage yani ortacha qiymatini chiqaradi

--24.
SELECT *
FROM Employees
WHERE Age < 30 AND DepartmentName IN ('HR', 'IT');
--bu yerda yoshi 30 dan kichik bolgan HR va IT bolimidagilar chiqadi ammo shu bolimda ishlaydigan 30 yoshdan kattalar chiqamaydi agar AND emas OR ga ozgartirsam hammasi chiqadi

--25.
SELECT *
FROM Customers
WHERE Email LIKE '%@gmail.com%';
--aytganimday %__%  foizlar ortasiga biror narsa yozilsa osha narsani qidiradi
